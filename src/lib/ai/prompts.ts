...
export function buildTranscriptChunkPrompt(input: {
  ...
  const isChineseOutput = /^zh\b/i.test(input.preferredLanguage);
  const genericQuestionLabel = isChineseOutput ? "主持人" : "Host";
  const genericAnswerLabel = isChineseOutput ? "嘉宾" : "Guest";
...
- If the identity is unclear, prefer stable generic labels like "${genericQuestionLabel}" and "${genericAnswerLabel}" and keep them consistent.
`
      : "";
...
3. Infer speakers conservatively. If uncertain, use generic labels like "${genericQuestionLabel}" and "${genericAnswerLabel}".
...
7. Never assign the same detected real person's name to both questionSpeaker and answerSpeaker in the same dialogue block unless the transcript is clearly a self-questioning monologue.
8. If only one side has a reliable real name, keep that name on that side and use "${genericQuestionLabel}" or "${genericAnswerLabel}" for the other side.
...
- For interview-style content, prefer "${genericQuestionLabel}" for the interviewer/moderator side and "${genericAnswerLabel}" for the unnamed respondent side.
${knownSpeakersPrompt}
